...
Whenever you start a new `Path`, you must inform the tracker. This resets the internal event state and prepares it for the new trajectory.
...
In your OpMode loop, calling `progressTracker.update()` alongside `follower.update()` evaluates all registered event markers and executes triggered actions automatically and safely.

Alternatively, if you are using a generated `SequentialCommandGroup`, you can query whether an event has triggered:
